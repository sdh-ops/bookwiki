...
export default function BestsellerPage() {
  ...
  useEffect(() => {
    if (activeTab === "current") {
      fetchAllData();
    } else if (activeTab === "trend" && !selectedTrendBook && !trendLoading) {
      async function loadDefaultTrendBook() {
        try {
          const { data } = await supabase
            .from("bw_bestseller_snapshots")
            .select(`
              book_id,
              bw_books!inner(id, title, author, publisher, isbn, cover_url)
            `)
            .eq("rank", 1)
            .order("snapshot_date", { ascending: false })
            .limit(1);

          if (data && data.length > 0 && data[0].bw_books) {
            loadBookTrend(data[0].bw_books);
          }
        } catch (err) {
          console.error("Failed to load default trend book:", err);
        }
      }
      loadDefaultTrendBook();
    }
